...
Кластер ClickHouse из 2 реплик с координацией через ZooKeeper (1 нода), развёрнутый через Docker Compose. Настроен для работы на ноутбуке с ограниченными ресурсами.
...
**Топология кластера (`company_cluster`):** 1 шард, 2 реплики с `internal_replication=true`.

## Требования

- Docker Engine 20.10+
- Docker Compose v2
- **Минимум 3 GB RAM** (рекомендуется 4 GB)
- **Минимум 2 CPU ядра**
...
|-------------|-----------|-----------|------------|------------|
| zk-01       | 0.30      | 256 MB    | 0.10       | 64 MB      |
| ch-01/02    | 0.75      | 1 GB      | 0.20       | 256 MB     |

**Суммарное потребление (максимум):** ~1.80 CPU, ~2.25 GB RAM.
...
├── clickhouse/
│   ├── config.d/
│   │   ├── cluster.xml                 # Топология кластера и ZooKeeper-нода
...
### Удаление объектов в кластере

```sql
DROP DATABASE IF EXISTS test_replication ON CLUSTER company_cluster SYNC;
DROP DATABASE IF EXISTS test_local SYNC;
```
Ключевое слово SYNC нужно при выполнении DDL-операций удаления (DROP, TRUNCATE), когда важно дождаться завершения операции на всех узлах перед продолжением.

Почему это необходимо

DDL-запросы с ON CLUSTER по умолчанию выполняются асинхронно: запрос помещается в очередь распределённых DDL (через ZooKeeper), и управление возвращается сразу, не    
дожидаясь выполнения на всех нодах. Это создаёт проблему, когда следующая операция зависит от завершения предыдущей.
...
- При заданных лимитах кластер стартует **15-30 секунд** (один ZooKeeper стартует быстрее ансамбля).
